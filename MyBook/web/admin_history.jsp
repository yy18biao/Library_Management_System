...
<%@ page import="entity.BookHistory" %>
...
<%
    // 从请求里获取数据集合
    List<BookHistory> list = (List<BookHistory>) request.getAttribute("list");
    // 集合为null，意味着没获取，直接跳转到do页面进行获取
    if (list == null) {
        request.getRequestDispatcher("BookServlet?op=getAllHistoryBook").forward(request, response);
        return;
    }
%>
<body>
...
<div id="main">
    <form action="BookServlet?op=getAllHistoryBook" method="post">
        输入读者名称查询：
...
<script>
    function del(id){
        if(confirm("是否删除？")){
            location.href="BookServlet?op=deleteHistroy&id="+id;
        }
    }
</script>