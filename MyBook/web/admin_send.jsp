...
<%
    ...
    if (list == null) {
        request.getRequestDispatcher("BookServlet?op=getAllNoReturnBook").forward(request, response);
        return;
    }
%>
<body>
...
<div id="main">
    <form action="BookServlet?op=getAllNoReturnBook" method="post">
        输入读者用户名查询：
    ...
    <table id="mytable" width="100%" cellspacing="0px" cellpadding="0px">
        ...
        <c:forEach items="${list}" var="cd" varStatus="status">
            <tr>
                ...
                <td>${cd.data }</td>
                <td>
                    <a href="BookServlet?op=return_book&id=${ cd.id }">归还</a>
                </td>
            </tr>
        </c:forEach>
    </table>
